-- Select from the household-level subqueries and aggregate to the cluster level
SELECT 
	...
	, COALESCE (min(locs.latitude)::text, 'NA') as latitude
	, COALESCE (min(locs.longitude)::text, 'NA') as longitude
	, CASE WHEN min(locs.latitude) IS NULL THEN 'NA' ELSE 'GPS' END as location_src
	...
	, SUM(person_nets.n_u5_nettbl) as n_pop_u5
	-- n children under 5 slept under ITN
	, SUM(person_nets.n_u5_under_itn) as n_u5_under_itn
	-- n pregnant women total
	, SUM(person_nets.n_preg_tot) as n_preg_tot
	-- n pregnant women slept under ITN
	, SUM(person_nets.n_preg_under_itn) as n_preg_under_itn
	-- n individuals that slept last night (de facto)
	, SUM(h0.hv013::Integer) as n_defacto_pop
	-- n de facto that slept under ITN
	, SUM(person_nets.n_tot_under_itn) as n_slept_under_itn
	-- n households
	...
	-- This has implications for household size etc if this was an error! :
	, sum(person_nets.hh_with_net_tbl_entry) as n_hh_in_net_tbl_tot
	-- mean hh size 
		-- this gives different res to old data (even accounting for different n households above). 
		-- The old data gave mean size of the households that were included in the nets table only.
	, AVG (h0.hv013::Float) as mean_hh_size
	-- Use COALESCE on all fields extracted via outer joins, so our output shows 0 rather than blanks/nulls
	-- n ITNs
	, COALESCE(sum(hh_nets.num_itn), 0) as n_itn
	-- n hh with ITNs
	, SUM (CASE WHEN COALESCE(hh_nets.num_itn, 0) > 0 THEN 1 ELSE 0 END) as n_hh_with_itn
	...
	, SUM (CASE WHEN COALESCE(hh_nets.theoretical_net_capacity, 0) >= h0.hv013::Integer 
		OR h0.hv013::Integer is null THEN 1 ELSE 0 END) as n_hh_with_enough_itn
	, SUM (CASE WHEN COALESCE(hh_nets.theoretical_net_capacity, 0) >= h0.hv013::Integer 
		AND h0.hv013::Integer > 0 THEN 1 ELSE 0 END) as n_occ_hh_with_enough_itn
	-- ITN capacity (2 people per ITN but capped at the actual hh pop)
	, SUM (LEAST (
		COALESCE(hh_nets.theoretical_net_capacity, 0), 
		(h0.hv013::Integer)
		)
	  ) as itn_usable_capacity
	-- n ITNs used
	, COALESCE(SUM(hh_nets.n_itn_used),0) as n_itn_used
-- ** TO CHECK
	-- n ITNs conventional
	, COALESCE(SUM (hh_nets.n_conv_itn), 0) as n_conv_itn
	-- n LLINs - this seems to match
	, COALESCE(SUM (hh_nets.n_llin), 0) as n_llin
	-- n LLINs under 1 yr old
	, COALESCE(SUM (hh_nets.n_llin_1yr), 0) as n_llin_1yr
	-- n LLINs 1-2 yrs
	, COALESCE(SUM (hh_nets.n_llin_1_2yr), 0) as n_llin_1_2yr
	-- n LLINs 2-3 yrs
	, COALESCE(SUM (hh_nets.n_llin_2_3yr), 0) as n_llin_2_3yr
	-- n LLINs 3+ yrs
	, COALESCE(SUM (hh_nets.n_llin_gt3yr), 0) as n_llin_gt3yr
	...
	LEFT OUTER JOIN
		...
		(SELECT 
			...
			, SUM (CASE WHEN hml10::Integer = 1
				...
				AND hml8::Integer <> 2 -- oops, was 1 -- treatment other than permanent (alternative location)
