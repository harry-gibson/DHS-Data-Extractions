...
The [sample_data](sample_data/) folder provides various examples of the input and output. 
- [input](sample_data/input/) contains a question-mapping file in which the questions have been appropriately flagged. This can be used as input to the SQL-generation workbench
- [output](sample_data/output) contains the SQL files generated by the workbench (under [generated_sql](sample_data/output/generated_sql)) and the CSV data files produced by executing these queries against the DHS database.