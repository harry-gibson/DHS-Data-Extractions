...
-- This query generates PR at a cluster level with NO WEIGHTS based on RDT DATA ONLY
--
...
--
-- This version is for RDT data only. From RDT data, the species is always assumed to be Pf.
-- For surveys which contain both RDT and MSY there is a choice over what we should do: at present we would
-- not run this RDT export if MSY data exist for a survey, but maybe we should do this too, modifying it 
-- to flag where "RDT confirmed by microscopy" is a thing, although that would be at the child not the cluster level.
--
...
WITH consts AS
	-- just putting these here so they can be manually edited more easily: get this info manually.
      ...
      , 5680              AS universal_dhs_id -- Internal MAP ID for DHS data in general
		  ...
		  , 'RDT'::text		    AS method -- 'RDT' for this version of the query
		)
SELECT
	NULL :: INT AS enl_id
	, NULL :: INT AS 
  ...
  -- source_id1 refers to the MAP biblio ID, paste this into the consts table at the end
	, min(consts.biblio_src_id) as source_id1
  -- source_id2 is the magic constant from the Book of Bonnie to be used for all DHS surveys in MAP PR db
  ...
  -- site_name came along so this is maybe redundant now and only here for compatibility with downstream processing
	...
	, sum(CASE WHEN hmh.hml35 IS NOT NULL THEN 1 ELSE 0 END) as examined
	-- and all this for one row, get the actual test result. RDT always counts as Pf
	, sum(CASE WHEN (hmh.hml35::Integer = 1) THEN 1 ELSE 0 END) as pf
  , '' as pv
  , '' as po
  , '' as pm
	...
	, sum(CASE WHEN (hmh.hml35::Integer = 1) THEN 1.0 ELSE 0.0 END)
		/
		sum(CASE WHEN hmh.hml35 IS NOT NULL THEN 1 ELSE 0 END)
		as pf_pr
  , '' as pv_pr
  , '' as po_pr
  , '' as pm_pr
